...
def softmax_loss(x, y):
    ...
    # 避免指数运算时数值溢出（如 exp(1000) 会超出浮点数范围）。
    # 对每个样本的分数减去该样本的最大值，使得所有分数 ≤ 0。
    # 数学性质：Softmax 的结果与输入分数整体偏移一个常数无关。
    ...
    # 使用 log_probs[torch.arange(N), y] 
    #   提取每个样本正确类别的对数概率。求和后取负数，再除以样本数 N，得到平均交叉熵损失。
