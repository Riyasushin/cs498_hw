          ...
          for f in range(F):  
            for i in range(Hp):  
                for j in range(Wp):  
                    ...
                    w_start = j * stride
                    w_end = w_start + WW
                    window = x_padded[n, :, h_start:h_end, w_start:w_end]  # Shape: (C, HH, WW)

                    # Compute gradients
                    # note: 给定已经是**d**out了
                    dout_val = dout[n, f, i, j] # dout_val：获取输出特征图 dout 在位置 (n, f, i, j) 的梯度值
                    db[f] += dout_val  # Gradient for bias
                    dw[f] += window * dout_val  # Gradient for weights
                    dx_padded[n, :, h_start:h_end, w_start:w_end] += w[f] * dout_val  # Gradient for input
