...
def hello():
    ...
    """
    print('Hello from pytorch101.py! new')
...
def multiples_of_ten(start: int, stop: int) -> Tensor:
    ...
    if st >= stop:
        ...
        x = torch.arange(st, stop, 10, dtype=torch.float64) 
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return x
...
def reverse_rows(x: Tensor) -> Tensor:
    ...
    len = x.shape[0]
    idx = list(range(len))
    idx[0], idx[len - 1] = idx[len - 1], idx[0]
    idx[1], idx[len - 2] = idx[len - 2], idx[1]
    y = x[idx]
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
...
def zero_row_min(x: Tensor) -> Tensor:
    ...
    # TODO : haha
    y = x.clone()
    min_indices = y.argmin(dim=1)
    rows = torch.arange(y.size(0))
...
def normalize_columns(x: Tensor) -> Tensor:
    ...
    #     mu_j = torch.sum(x[:, j]) / x.shape[0]
    #     sigma_j = torch.sqrt(torch.sum((x[:, j] - mu_j).pow(2)) / (x.shape[0] - 1))
    #     y[:, j] = (x[:, j] - mu_j) / sigma_j
     # 计算每列的和

    column_sums = torch.sum(x, dim=0)  #column_sums  Tensor(N)
    print(column_sums)
    mu = column_sums / x.shape[0] # 全部除M
    squared_diff = (x - mu).pow(2) # 这里发生了broadcast Tensor(M, N)
    column_var_sums = torch.sum(squared_diff, dim=0) 
    sigma = torch.sqrt(column_var_sums / (x.shape[0] - 1)) # Tensor(M)
    y = (x - mu) / sigma # broadcast
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
