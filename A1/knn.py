...
def compute_distances_two_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    # dists = (x_train_flat.unsqueeze(1) - x_test_flat).pow(2).sum(-1)
    # dists = dists.cpu()
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    x_train_flat = x_train.view(num_train, -1)
    x_test_flat = x_test.view(num_test, -1)
    x_train_pow = torch.sum(x_train_flat ** 2, dim=1, keepdim=True)
    # 插入新维度：unsqueeze(0) 在第 0 维插入一个新的维度，将一维的 (num_test,) 张量转换为二维的 (1, num_test) 张量。
    # 这样做是为了后续在计算平方欧几里得距离时能够利用广播机制与其他张量进行正确的运算。
    print(' ' + str(torch.sum(x_test_flat ** 2, dim=1).shape))
    print(' ' + str(torch.sum(x_test_flat ** 2, dim=1).unsqueeze(0).shape))
    x_test_pow = torch.sum(x_test_flat ** 2, dim=1).unsqueeze(0)
    x_point = torch.mm(x_train_flat, x_test_flat.t())
    dists = x_train_pow + x_test_pow - 2 * x_point
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    for j in range(num_test):
        ...
        # 获取出现次数最多的标签
        y_pred[j] = unique_labels[max_count_index]
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return y_pred
...
class KnnClassifier:
    ...
    def predict(self, x_test: torch.Tensor, k: int = 1):
        ...
        num_test = x_test.shape[0]
        x_train_flat = self.x_train.view(num_train, -1)
        x_test_flat = x_test.view(num_test, -1)
        x_train_pow = torch.sum(x_train_flat ** 2, dim=1, keepdim=True)
        # 插入新维度：unsqueeze(0) 在第 0 维插入一个新的维度，将一维的 (num_test,) 张量转换为二维的 (1, num_test) 张量。
        # 这样做是为了后续在计算平方欧几里得距离时能够利用广播机制与其他张量进行正确的运算。
        ...
        for j in range(num_test):
            _, indices = torch.topk(dists[:, j], k, largest=False)
            k_near_labels = self.y_train[indices]
            labels, counts = torch.unique(k_near_labels, return_counts=True)
            max_count_index = torch.argmax(counts)
            y_test_pred[j] = labels[max_count_index]
...
def knn_cross_validate(
    ...
    for k in range(len(k_choices)):
        ...
        for i in range(num_folds):
            ...
            test_indices = [j for j in range(num_folds) if j != i] # in python is jump over i!!!
            x_val_fold = torch.cat([x_train_folds[j] for j in test_indices])
            y_val_fold = torch.cat([y_train_folds[j] for j in test_indices])

            accuracy = classifier.check_accuracy(x_val_fold, y_val_fold, k=k_choices[k], quiet=True)
...
def knn_get_best_k(k_to_accuracies: Dict[int, List]):
    ...
    for k, accuracies in k_to_accuracies.items():
        ...
        # 如果当前平均准确率大于之前记录的最佳平均准确率，或者平均准确率相等但 k 值更小
